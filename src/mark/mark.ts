...
import { isPosition } from '../runtime/scale';
import { createColumnOf } from '../runtime/mark';
...
export function maybeVisualChannel(
  ...
  const newEncode = {};
  const style = {};
  for (const [name, channel] of Object.entries(encode)) {
    const { type } = channel;
    if (type !== 'constant' || isPosition(name)) newEncode[name] = channel;
    else style[name] = channel.value;
  }
  return [I, { ...mark, encode: newEncode, style }];
}
